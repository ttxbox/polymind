...
import { BUILT_IN_TOOL_SERVER_NAME } from '../builtInToolsPresenter'
        ...
        try {
          ...
          if (needContinueConversation && currentToolCalls.length > 0) {
            for (const toolCall of currentToolCalls) {
              ...
              const toolDef = availableTools.find((t) => t.function.name === toolCall.name)
              ...
              const isBuiltInTools = toolDef.server?.name === BUILT_IN_TOOL_SERVER_NAME
              ...
              try {
                ...
                if (supportsFunctionCallInAgent && !isBuiltInTools) {
