...
Welcome to PolyMind! This guide will walk you through installing PolyMind, configuring its features, and using it to interact with various Large Language Models (LLMs).
...
To get started with PolyMind, download the latest version for your operating system from the [Gitee Releases page](https://gitee.com/openeuler/polymind/releases).
...
PolyMind supports various cloud LLMs, including:
- DeepSeek
...
PolyMind has integrated support for Ollama, allowing you to manage and use local models without command-line operations.
...
PolyMind supports a multi-window and multi-tab architecture, similar to a web browser. This allows for:
...
-   PolyMind provides complete Markdown rendering for chat messages.
...
-   PolyMind supports displaying multi-modal content within chats.
...
-   PolyMind supports Artifacts rendering, which provides diverse ways to present results from LLMs, especially when using Tool Calling (MCP).
...
PolyMind enhances LLM responses by integrating with search engines. This provides more accurate, timely, and verifiable information.
...
PolyMind features excellent Model Context Protocol (MCP) support, allowing LLMs to use tools and access external resources.
...
PolyMind prioritizes user privacy and data security.

### Screen Projection Hiding

-   To prevent sensitive information from being accidentally displayed during screen sharing or projections, PolyMind offers a screen projection hiding feature. This typically obscures the chat content when active.

### Network Proxies

-   You can configure PolyMind to use network proxies, adding an extra layer of privacy and potentially bypassing network restrictions.

### Data Storage and Encryption

-   Chat data and configuration data have reserved encryption interfaces.
-   PolyMind focuses on local data storage where possible to reduce the risk of information leakage.

## DeepLink Support

PolyMind utilizes rich DeepLink support, which allows:
